...
const FormInput = (props) => {
    ...
    const { label, errorMessage, onChange, id, addressSuggestion, inputName, values, onAddressChange, showSuggestion, setShowSuggestion, location, ...inputProps } = props;
    ...
    useEffect(() => {
        if ((inputName === "start_address" || inputName === "destination_01" || inputName === "destination_02" || inputName === "destination_03" || inputName === "destination_04" || inputName === "destination_05") && values) {
            setShowSuggestion(true);
        }
    }, [inputName, values, setShowSuggestion]);
    return (
            ...
            <div className={"card border-top-0 rounded-lg mb-2 inputSugesstions"}>
                ...
                {(showSuggestion && addressSuggestion[inputName].items.length > 0) &&
                    <div>
                        ...
                        {location.isLocationOn !== false &&
                            <div className="card-body ps-3 py-1 m-1 border border-light rounded inputSugesstion">
                                <h6 onClick={(e) => onAddressChange(e, location, inputName)} className="" ><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-compass-fill" viewBox="0 0 16 16">
